 ...
 */
const anItem = (id: string, season: number | null, episode: number | null, title = '') => ({
  id,
  season,
  episode,
  title,
...
describe('fileDownload', () => {
  it('files a film by its largest video, with its subtitles, and leaves the sample', async () => {
    ...
    expect(filed.get('film')).toBe(join(library, 'Dune (2021)', 'Dune (2021).mkv'));
  ...
  it('files each episode of a season pack by the numbers in its name', async () => {
    ...
    expect(filed.get('2')).toBe(
      join(library, 'Severance (2022)', 'Season 01', 'Severance (2022) - S01E02.mkv'),
    );
  });
