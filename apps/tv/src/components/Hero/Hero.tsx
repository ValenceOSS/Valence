 ...
 * the page has settled, with everything needed to decide on it — its lettering, the year and what it
 * is, how it looks and sounds, what it is about — and the two things somebody might do: watch it, or
 * open its page first.
 *
 * A handful of titles take turns, the dots filling as each turn runs, but never while the remote is
 * on the hero's buttons, since the thing somebody is about to press should not change beneath them. Nothing plays
 * while another page covers the front page.
 *
 ...
 */
const Hero = ({ items, progress, isCovered, onPlay, onInspect, onFeature }: HeroProps) => {
      ...
      <FadeIn key={media.id}>
        ...
        <View style={styles.words}>
          ...
          <View style={styles.factsRow}>
            ...
            <Badges
              badges={
                detail.data === undefined || detail.data === null ? [] : qualityBadges(detail.data)
              }
            />
          </View>

          {overview === null || isSeries ? null : (
