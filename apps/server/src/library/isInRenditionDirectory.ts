...
/**
 * Whether a path sits in the directory Valence keeps its own files in, beside the film they were
 * made from.
 *
 * The scanner walks a library root and upserts every media file it finds, so an encode written
 * beside the original would be indexed as a second copy of the film — its own metadata lookup, its
