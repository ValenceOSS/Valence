...
describe('aDiscordActivity', () => {
  it('puts the programme on the top line, which is the one Discord draws largest', () => {
    expect(aDiscordActivity(AN_EPISODE)).toMatchObject({
      details: 'A Programme',
    });
  });
  ...
  it('gives a film one line, since it belongs to no programme', () => {
    const activity = aDiscordActivity(A_FILM);
  ...
  it('says it is watching rather than playing, which is a different word in Discord', () => {
    expect(aDiscordActivity(A_FILM)).toMatchObject({ type: 3 });
  });

  it('sends the times in seconds, so Discord counts for itself rather than being told', () => {
    expect(aDiscordActivity(AN_EPISODE)?.timestamps).toEqual({
      start: 1_755_000_000,
      end: 1_755_001_400,
    });
  });
  ...
  it('sends no end for something whose length is not known', () => {
    expect(aDiscordActivity(A_FILM)?.timestamps).toEqual({
      start: 1_755_000_000,
    });
  });

  it('draws the Valence logo where the catalogue has no picture to draw', () => {
    expect(aDiscordActivity(A_FILM)?.assets).toMatchObject({
  ...
  it('wears no badge while watching, since the logo already says which application this is', () => {
    expect(aDiscordActivity(AN_EPISODE)?.assets).not.toHaveProperty('small_image');
    expect(aDiscordActivity(AN_EPISODE)?.assets).not.toHaveProperty('small_text');
  });

  it('wears no badge while browsing either', () => {
    expect(aDiscordActivity({ kind: 'browsing' })?.assets).not.toHaveProperty('small_image');
    expect(aDiscordActivity({ kind: 'browsing' })?.assets).not.toHaveProperty('small_text');
  });

  it('offers a way to look a film up, pointed at the right kind of page', () => {
    expect(aDiscordActivity(A_FILM)?.buttons).toEqual([
  ...
  it('counts towards nothing while browsing, since there is nothing to count towards', () => {
    expect(aDiscordActivity({ kind: 'browsing' })).not.toHaveProperty('timestamps');
  });

  it('offers no button while browsing, since there is nowhere in particular to send anybody', () => {
    expect(aDiscordActivity({ kind: 'browsing' })).not.toHaveProperty('buttons');
  });
  ...
  it('draws the catalogue picture where there is one, which is what somebody recognises', () => {
    ...
    expect(aDiscordActivity({ ...AN_EPISODE, artwork: poster })?.assets).toMatchObject({
      large_image: poster,
    });
  });
  ...
  it('refuses a picture from anywhere else, which would publish where somebody keeps their server', () => {
    ...
    expect(aDiscordActivity({ ...AN_EPISODE, artwork: mine })?.assets).toMatchObject({
      large_image: 'valence-desktop',
    });
  });
  ...
  it('refuses a picture sent without encryption, wherever it came from', () => {
    ...
    expect(aDiscordActivity({ ...AN_EPISODE, artwork: plain })?.assets).toMatchObject({
      large_image: 'valence-desktop',
    });
  });

  it('says which application this is when somebody rests on the picture, not which version', () => {
    expect(aDiscordActivity(AN_EPISODE)?.assets.large_text).toBe('Valence');
  });

  it('sends a watch party as a group, so it draws the figure and the count', () => {
    expect(aDiscordActivity({ ...AN_EPISODE, party: { id: 'a-party', size: 3 } })?.party).toEqual({
      id: 'a-party',
      size: [3, 3],
    });
  });
  ...
  it('sends no group for somebody watching on their own', () => {
    expect(aDiscordActivity(AN_EPISODE)).not.toHaveProperty('party');
  });

  it('marks a pause rather than taking the status down, since a pause is still watching', () => {
    expect(aDiscordActivity({ ...AN_EPISODE, isPaused: true })?.state).toBe(
      'Paused — Series 2, Episode 12',
    );
  });

  it('says so plainly for a film, which has no line of its own to add it to', () => {
    expect(aDiscordActivity({ ...A_FILM, isPaused: true })?.state).toBe('Paused');
  });

  it('takes the clock off while paused, which would otherwise count towards an end not coming', () => {
    expect(aDiscordActivity({ ...AN_EPISODE, isPaused: true })).not.toHaveProperty('timestamps');
  });

  it('keeps what is playing on the status while paused, so it is still recognisable', () => {
    expect(aDiscordActivity({ ...AN_EPISODE, isPaused: true })?.details).toBe('A Programme');
  });

  it('says who somebody is watching with, in the line they read rather than a field they will not see', () => {
    expect(aDiscordActivity({ ...AN_EPISODE, party: { id: 'a-party', size: 2 } })?.state).toBe(
      'Series 2, Episode 12 · with 1 other',
    );
  });

  it('counts the others rather than the party, since somebody knows they are in it', () => {
    expect(aDiscordActivity({ ...AN_EPISODE, party: { id: 'a-party', size: 4 } })?.state).toBe(
      'Series 2, Episode 12 · with 3 others',
    );
  });

  it('says the company for a film, which has no line of its own to add it to', () => {
    expect(aDiscordActivity({ ...A_FILM, party: { id: 'a-party', size: 3 } })?.state).toBe(
      'with 2 others',
    );
  });

  it('says nothing about company for a party of one, which is watching alone', () => {
    expect(aDiscordActivity({ ...AN_EPISODE, party: { id: 'a-party', size: 1 } })?.state).toBe(
      'Series 2, Episode 12',
    );
  });
  ...
  it('keeps both the pause and the company, which are true at the same time', () => {
    expect(
      aDiscordActivity({ ...AN_EPISODE, isPaused: true, party: { id: 'a-party', size: 2 } })?.state,
  ...
  it('says it is listening rather than watching, which is a different word in Discord', () => {
    expect(aDiscordActivity(A_TRACK)).toMatchObject({ type: 2 });
  });

  it('puts the track on the top line', () => {
    expect(aDiscordActivity(A_TRACK)).toMatchObject({
      details: 'How Not To Drown',
    });
  });

  it('wears a badge over its own cover, since the cover no longer says which application this is', () => {
    expect(aDiscordActivity(A_TRACK)?.assets).toMatchObject({
  ...
  it('badges a pause, so a still status is not mistaken for a stuck one', () => {
    expect(aDiscordActivity({ ...A_TRACK, isPaused: true })?.assets).toMatchObject({
      small_text: 'Paused',
    });
  });

  it('names who made it below the track, the way somebody would say it aloud', () => {
    expect(aDiscordActivity(A_TRACK)?.state).toBe('CHVRCHES & Robert Smith');
  });

  it('names who made it in the header too, rather than saying which application this is', () => {
    expect(aDiscordActivity(A_TRACK)?.name).toBe('CHVRCHES & Robert Smith');
  });

  it('joins three or more names the way somebody would say them aloud', () => {
    expect(aDiscordActivity({ ...A_TRACK, artists: ['A', 'B', 'C'] })?.state).toBe('A, B & C');
  });
  ...
  it('draws the catalogue cover for a track, from the server actually holding it', () => {
    ...
    expect(aDiscordActivity({ ...A_TRACK, artwork: cover })?.assets).toMatchObject({
      large_image: cover,
    });
  });
  ...
  it('refuses a track cover sent without encryption, wherever it came from', () => {
    ...
    expect(aDiscordActivity({ ...A_TRACK, artwork: plain })?.assets).toMatchObject({
      large_image: 'valence-desktop',
    });
  });

  it('sends the times in seconds, so Discord counts down the track for itself', () => {
    expect(aDiscordActivity(A_TRACK)?.timestamps).toEqual({
      start: 1_755_000_000,
      end: 1_755_000_331,
    });
  });
  ...
  it('says who else is listening in the line somebody reads', () => {
    expect(aDiscordActivity({ ...A_TRACK, party: { id: 'a-party', size: 2 } })?.state).toBe(
