...
Native code is allowed in one place and grudgingly: a module under
`apps/ios/modules/`, written in Swift, where a platform capability has no
binding and no maintained library provides one. Nothing outside `apps/ios` may
depend on it, and a native module is its own change with its own
justification — the same bar as lowering a coverage threshold.
