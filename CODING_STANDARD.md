...
controls are native APIs that a web view cannot reach — and because a
repackaged website is rejected from the App Store.

The Xcode project is generated by `expo prebuild` and is `.gitignore`d. It is a
build artifact and is never hand-edited, for the same reason generated
JavaScript config is.

What is not rebuilt is the part that would rot. `@valence/client`,
`@valence/contracts` and `@valence/core` are drawing-free and run unchanged on
a phone, so the readers, queries, schemas and session live in one place and
stay there. The screens are a second implementation because React Native draws
...
### Kotlin, for Android's own capabilities and nothing else
...
`AudioManager`. The Android side of a module is written in the language Android
declares it in, for the same reason its Apple side is Swift.

The same boundary holds as for Swift:

- The Android side of a module offers the same functions and events as its
  Apple side, under the same name, so TypeScript reaches either one the same way
  and never asks which it has.
- It wraps one system capability. It holds no screens, no application state and
  no requests to the server of its own devising.
- Each module is declared by its `expo-module.config.json` and the
  `build.gradle` Gradle reads. The build file is Groovy because the Expo module
  plugin is applied from Groovy; it declares the module and its dependencies and
  does nothing else.
- The Android project an app is built from is generated by `expo prebuild` and
  is `.gitignore`d, like the Xcode project.
