import { render, screen } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import { describe, expect, it, vi } from 'vitest';
import { AskableMusicTile } from './AskableMusicTile';
import type { CatalogueTitle } from '@ValenceContracts/schemas/CatalogueTitle';

const ALBUM: CatalogueTitle = {
  kind: 'album',
  id: 'deezer-7',
  title: 'Pylon',
  subtitle: 'Band',
  year: null,
  overview: null,
  posterUrl: null,
  standing: { status: 'requested', mediaId: null, requestId: null, requestState: 'searching' },
};

describe('AskableMusicTile', () => {
  it('says who it is by and where it stands, and opens what is chosen', async () => {
    const onAsk = vi.fn();

    render(<AskableMusicTile title={ALBUM} onAsk={onAsk} />);

    expect(screen.getByText('Band · Requested')).toBeInTheDocument();

    await userEvent.click(screen.getByRole('button', { name: /Pylon/ }));

    expect(onAsk).toHaveBeenCalledWith('album:deezer-7');
  });

  it('sets a display name so devtools can identify it', () => {
    expect(AskableMusicTile.displayName).toBe('AskableMusicTile');
  });
});
